...
export async function putUser(req, res) {
  try {
    console.log("putUser called with body:", req.body);
    const { fullName, profilePic, bio, location, language } = req.body;

    const updatedUser = await User.findByIdAndUpdate(
      req.user.id,
      { fullName, profilePic, bio, location, language },
    ...
    console.error("Error in putUser controller", error.message);
...
export async function unFriend(req, res) {
  try {
    const { userId, requestId } = req.params; // userId = friend ID, requestId = FriendRequest ID (if any)
    const currentUserId = req.user.id;
    ...
    // 2. Remove each other from friends list
    ...
    // 3. Remove friend request if requestId is valid
    if (requestId && requestId !== "null" && requestId !== "undefined") {
      await FriendRequest.findByIdAndDelete(requestId);
    } else {
      // Optional fallback: clean up any pending friend requests between them
...
//       .select('fullName profilePic  learningLanguage')
//       .lean();
